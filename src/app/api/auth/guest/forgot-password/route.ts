...
import { GuestForgotPasswordSchema } from '@/server/validators';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateCheck = GuestRateLimiter.isRateLimited(`forgot:${ip}`);
    ...
    const validated = GuestForgotPasswordSchema.parse(body);

    await GuestPasswordResetService.requestReset(validated.email, ip);
